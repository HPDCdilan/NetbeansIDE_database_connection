...
public class DataApp {
	public static void main (String args[]){ // to execute
		...
		Class.forName("com.mysql.jdbc.Driver"); // loading the MS Access driver

		System.out.println("Connecting");

 		con = DriverManager.getConnection("jdbc:mysql://localhost:3306/letme","root","");
		// odbc_ex is the data source name (DSN)

 		System.out.println("Connected");

 		// the transaction to be carried out - SQL statement
 		String queryString = "insert into Names (id, first_name, last_name) values('007','James','Bond')";

		//System.out.println(queryString);
		 stmt = con.createStatement(); // creating the statement object to work with database

 		int i = stmt.executeUpdate(queryString); // returns an integer - number of records added

        	if (i != 0){// cheack wether data updating or not
